#include<stdio.h>
void shellsort(int a[],int interval,int size){//有头结点a[0]的线性表正向希尔排序 
	int i,j;
...
void shellsort2(int a[],int interval,int size){//无头结点a[0]的线性表正向希尔排序
	int i,j;
	for(i=interval;i<size;i++){//每个间隔（interval）一定距离的元素之间进行正向简单插入排序
		if(a[i]<a[i-interval]){
			//如果碰到逆序，就执行在这些规律间隔的元素中的正向简单插入排序（如下） 
			int temp=a[i];
